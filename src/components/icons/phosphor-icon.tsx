"use client";

import * as PhosphorIcons from "@phosphor-icons/react";
import type { IconProps } from "@phosphor-icons/react";
import { ICON_MAP, type LucideIconName } from "./icon-map";
import { iconTokens, type IconSize, type IconWeight } from "@/lib/design-tokens";

interface PhosphorIconProps {
  name: LucideIconName;
  className?: string;
  size?: IconSize | number;
  weight?: IconWeight;
  "aria-hidden"?: boolean;
}

export function PhosphorIcon({
  name,
  className,
  size = "md",
  weight = iconTokens.defaultWeight,
  "aria-hidden": ariaHidden = true,
  ...props
}: PhosphorIconProps) {
  const phosphorName = ICON_MAP[name];
  const IconComponent = PhosphorIcons[
    phosphorName as keyof typeof PhosphorIcons
  ] as React.ComponentType<IconProps>;
  if (!IconComponent) {
    console.warn(`Phosphor icon not found for: ${name}`);
    return null;
  }
  const iconSize = typeof size === "number" ? size : iconTokens.sizes[size];
  return (
    <IconComponent
      className={`shrink-0 ${className ?? ""}`.trim()}
      size={iconSize}
      weight={weight}
      aria-hidden={ariaHidden}
      {...props}
    />
  );
}
